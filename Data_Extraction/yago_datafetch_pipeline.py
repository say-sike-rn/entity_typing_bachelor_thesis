...
# Used on the very first run (no subclasses registered yet): fetches layer 1+2. 
# (NOTE: This ended up not affecting the end results at all, as the hierarchical structure of the resulting dataset
# was modeled later on based on the subclass registry and all classes were had direct relations, forming graph that would 
# have looked the same if this bootstap run hadn't included both the first and second layer.)
...
WHERE {
    ...
    UNION
    {
        ?subclass rdfs:subClassOf/rdfs:subClassOf schema:Person .
        ?instance a ?class .
        ?class rdfs:subClassOf* ?subclass .
    }
}
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 3 – Filter _Q### cryptic names because they are saved differently in YAGO than in Wikipedia despite being the same entities.
# ─────────────────────────────────────────────────────────────────────────────
    ...
    for attempt in range(_WIKI_MAX_RETRIES):
        ...
        try:
            ...
            if response.status_code == 429:
                backoff = _WIKI_BACKOFF_BASE * (2 ** attempt)
...
def step8_make_bert(run: "PipelineRun", in_path: Path) -> Path:
    ...
    run_depth = 1 if is_bootstrap else 2  # ignore this. Depth is determined later on in another script.
